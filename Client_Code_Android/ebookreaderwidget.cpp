    ...
    m_totalPages(0)
{
    ...
    qDebug() << "[EBookReaderWidget] Android-specific styles applied";
#endif
...
void EBookReaderWidget::loadPage(int pageNum)
{
    ...
    // m_pagesDir已经包含完整的assets路径（如"assets:/PDF/pages"）
    if (m_pagesDir.startsWith("assets:")) {
        possiblePaths << m_pagesDir + "/" + filename;
        qDebug() << "[EBookReaderWidget] Android platform, using assets path:" << m_pagesDir;
    } else {
        // 如果不是assets路径，尝试构建assets路径
        possiblePaths << "assets:/PDF/" + m_pagesDir + "/" + filename
                      << "assets:/PDF/pages/" + filename;
        qDebug() << "[EBookReaderWidget] Android platform, trying to construct assets paths";
    }
#endif
    ...
    if (pixmap.isNull()) {
        ...
        // 动态计算最大宽度（窗口宽度 - 边距）
        int maxWidth = this->width() - 20;  // 左右各10px边距
        if (maxWidth < 300) maxWidth = 300;  // 最小宽度300px
