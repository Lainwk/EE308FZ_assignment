    ...
    ui(new Ui::ContactsPage)
{
    ...
    // 设置标题和标签样式
    ui->labelTitle->setStyleSheet("font-size: 16px; font-weight: bold; color: #000000;");
    ui->label->setStyleSheet("font-size: 16px; font-weight: bold; color: #000000;");
    ui->label_2->setStyleSheet("font-size: 16px; font-weight: bold; color: #000000;");
    
    // 设置搜索框样式
    ui->editSearch->setStyleSheet("font-size: 14px; color: #000000; padding: 8px;");
    
    qDebug() << "[ContactsPage] Android-specific styles applied";
#endif
...
#ifdef Q_OS_ANDROID
        // Android平台：明确设置文字样式，确保可见
        QFont itemFont;
        itemFont.setFamily("Roboto");
        itemFont.setPixelSize(18);  // 增大字体
        itemFont.setBold(true);
        item->setFont(itemFont);
        item->setForeground(QBrush(QColor("#000000")));  // 明确黑色
        item->setBackground(QBrush(QColor("#FFFFFF")));  // 白色背景
        
        qDebug() << "[ContactsPage] Added friend item:" << f.name << "with Android styles";
#endif
...
#ifdef Q_OS_ANDROID
    // 设置列表样式
    ui->listFriends->setStyleSheet(
        "QListWidget { background: white; border: none; }"
        "QListWidget::item { padding: 8px; border-bottom: 1px solid #eee; color: #000000; }"
